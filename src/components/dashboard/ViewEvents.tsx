...
export function ViewEvents() {
  ...
  const [itemToEdit, setItemToEdit] = React.useState<Event | null>(null);
  const [editFormData, setEditFormData] = React.useState<EditFormData>({ title: "", location: "", date: "" });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "events", itemToDelete.id));
  ...
  const handleEditClick = (item: Event) => {
    setItemToEdit(item);
    setEditFormData({ title: item.title, location: item.location, date: item.date });
  ...
  const handleUpdateItem = async (e: React.FormEvent) => {
    ...
    setIsUpdating(true);
    const itemRef = doc(db, "events", itemToEdit.id);
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>A list of past and upcoming club events.</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the event titled "<b>{itemToDelete?.title}</b>".
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <form id="edit-event-form" onSubmit={handleUpdateItem}>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
               ...
               <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="location" name="location" value={editFormData.location} onChange={handleFormChange} className="col-span-3" required/>
              </div>
            </div>
          </form>
           <DialogFooter>
              <Button type="submit" form="edit-event-form" disabled={isUpdating}>
